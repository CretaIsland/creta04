...
// import 'dartcolors.dart'; //D:\Flutter\src\flutter\packages\flutter\lib\src\cupertino\slider.dart
// import 'theme.dart';
...
class CretaCupertinoSlider extends StatefulWidget {
  ...
  @override
  State<CretaCupertinoSlider> createState() => _CupertinoSliderState();
...
class _CupertinoSliderState extends State<CretaCupertinoSlider> with TickerProviderStateMixin {
...
const double _skparkRadius = 10.0; // CupertinoThumbPainter.radius
...
class _RenderCupertinoSlider extends RenderConstrainedBox implements MouseTrackerAnnotation {
    ...
    MouseCursor cursor = MouseCursor.defer,
        ...
        super(
            additionalConstraints:
                const BoxConstraints.tightFor(width: _kSliderWidth, height: _kSliderHeight)) {
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    if (isInteractive) {
      final double extent =
          math.max(_kPadding, size.width - 2.0 * (_kPadding + CupertinoThumbPainter.radius));
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (visualPosition > 0.0) {
      ...
      canvas.drawRRect(
          RRect.fromLTRBXY(trackLeft, trackTop, trackActive, trackBottom, 1.0, 1.0), paint);
    }

    if (visualPosition < 1.0) {
      final Paint paint = Paint()..color = leftColor;
      canvas.drawRRect(
          RRect.fromLTRBXY(trackActive, trackTop, trackRight, trackBottom, 1.0, 1.0), paint);
    }

    final Offset thumbCenter = Offset(trackActive, trackCenter);
    CupertinoThumbPainter(color: thumbColor)
        .paint(canvas, Rect.fromCircle(center: thumbCenter, radius: _skparkRadius));
  }
