...
class TreeViewController {
  ...
  TreeViewController withAddNode<T>(
    ...
    List<Node> _data = addNode<T>(key, newNode, parent: parent, mode: mode, index: index);
    return TreeViewController(
      children: _data,
      selectedKey: selectedKey,
    );
  ...
  Node<T>? getNode<T>(String key, {Node? parent}) {
    ...
    while (iter.moveNext()) {
      ...
      if (child.key == key) {
        ...
        if (child.isParent) {
          _found = getNode<T>(key, parent: child);
          if (_found != null) {
            break;
          }
        }
  ...
  List<Node> expandAll({Node? parent}) {
    List<Node> _children = [];
    Iterator iter = parent == null ? children.iterator : parent.children.iterator;
    while (iter.moveNext()) {
      Node child = iter.current;
      if (child.isParent) {
  ...
  Node<T>? getParent<T>(String key, {Node? parent}) {
    ...
    while (iter.moveNext()) {
      ...
      if (child.key == key) {
        ...
        if (child.isParent) {
          _found = getParent<T>(key, parent: child);
          if (_found != null) {
            break;
          }
        }
  ...
  List<Node> addNode<T>(
    ...
    List<Node> __children = parent == null ? children : parent.children;
